...
def test_proxy_apt_configuration(host):
    ...
    if 'http' in proxy_proxies:
        assert f"Acquire::http::proxy \"{proxy_proxies['http']}\";" in proxy_file.content_string
    if 'https' in proxy_proxies:
        assert f"Acquire::https::proxy \"{proxy_proxies['https']}\";" in proxy_file.content_string
    if 'ftp' in proxy_proxies:
        assert f"Acquire::ftp::proxy \"{proxy_proxies['ftp']}\";" in proxy_file.content_string
